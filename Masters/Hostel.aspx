<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/AdminMaster.master"
    AutoEventWireup="true" CodeFile="Hostel.aspx.cs" Inherits="Hostel" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            //        GetHostel Function on page load
        ...
        function GetHostel(pageIndex) {
        if ($("[id*=hfViewPrm]").val() == 'true') {
            $.ajax({
                type: "POST",
                url: "../Masters/Hostel.aspx/GetHostel",
        ...
        //        GetHostel On Success Function
        //        Get Hostel to Grid

        function OnSuccess(response) {
            var xmlDoc = $.parseXML(response.d);
            var xml = $(xmlDoc);
            var Hostels = xml.find("Hostels");
            var row = $("[id*=dgHostel] tr:last-child").clone(true);
            $("[id*=dgHostel] tr").not($("[id*=dgHostel] tr:first-child")).remove();
            var eanchor = ''
            var eanchorEnd = '';
            var danchor = ''
            var danchorEnd = '';
            if ($("[id*=hfEditPrm]").val() == 'false') {
                ...
                eanchor = "<a  href=\"javascript:EditHostel('";
                eanchorEnd = "');\">Edit</a>";
            }
            if ($("[id*=hfDeletePrm]").val() == 'false') {
                ...
                danchor = "<a  href=\"javascript:DeleteHostel('";
                danchorEnd = "');\">Delete</a>";
            }
            if (Hostels.length == 0) {
                $("td", row).eq(0).html("");
                ...
                $("td", row).eq(3).html("");
                $("[id*=dgHostel]").append(row);
                row = $("[id*=dgHostel] tr:last-child").clone(true);

            }
            else {
                $.each(Hostels, function () {
                    var Hostel = $(this);
                    var ehref = eanchor + $(this).find("HostelID").text() + eanchorEnd;
                    var dhref = danchor + $(this).find("HostelID").text() + danchorEnd;
        ...
        function DeleteHostel(id) {
            var parameters = '{"HostelID": "' + id + '"}';
       if (jConfirm('Are you sure to delete this?', 'Confirm', function (r) {
                if (r) {
                $.ajax({

                    type: "POST",
                    url: "../Masters/Hostel.aspx/DeleteHostel",
                    data: parameters,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
        ...
        if ($("[id*=hfEditPrm]").val() == 'true') {
            ...
            $.ajax({
                type: "POST",
                url: "../Masters/Hostel.aspx/EditHostel",
        ...
        function OnEditSuccess(response) {
            var xmlDoc = $.parseXML(response.d);
            var xml = $(xmlDoc);
            var Hostel = xml.find("EditHostel");
            $.each(Hostel, function () {
                var Hostel = $(this);
                $("[id*=txtHostelName]").val($(this).find("HostelName").text());
                $("[id*=hfHostelID]").val($(this).find("HostelID").text());
        ...
        // Save Hostel
        function SaveHostel() {
            if (($("[id*=hfAddPrm]").val() == 'true' && $("[id*=hfHostelID]").val() == '') ||
            ($("[id*=hfEditPrm]").val() == 'true' && $("[id*=hfHostelID]").val() != '')
            ) {
                if ($('#aspnetForm').valid()) {
                    $("[id*=btnSubmit]").attr("disabled", "true");
                var HostelID = $("[id*=hfHostelID]").val();
                var HostelName = $("[id*=txtHostelName]").val();
                var parameters = '{"id": "' + HostelID + '","name": "' + HostelName + '"}';
                $.ajax({
                    type: "POST",
                    url: "../Masters/Hostel.aspx/SaveHostel",
                    data: parameters,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
        ...
        function OnSaveSuccess(response) {
            ...
            if (response.d == "Updated") {
               AlertMessage('success', 'Updated');
                GetHostel(parseInt(currentPage));
                Cancel();
            }
        ...
        $(".Pager .page").live("click", function (e) {
            GetHostel(parseInt($(this).attr('page')));
        });

        function Cancel() {
            $("[id*=txtHostelName]").val("");
            $("[id*=hfHostelID]").val("");
                ...
                <table width="100%">
                            ...
                            <div id="dvCashVoucher" style="float: left; width: 650px" runat="server">
                                <table class="form">
                                    <tr>
                                        ...
                                        <td class="col2">
                                            <asp:TextBox ID="txtHostelName" CssClass="jsrequired bloodgroup" runat="server"></asp:TextBox>
                                        </td>
                                        <td rowspan="3">
                                            <div class="block">
                                            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                         <button id="btnSubmit" type="button" class="btn-icon btn-orange btn-saving"  onclick="SaveHostel();" ><span></span><div  id="spSubmit">Save</div></button>
                    ...
                    <tr valign="top">
                        <td valign="top" colspan="2">
                            <asp:GridView ID="dgHostel" runat="server" Width="100%" AutoGenerateColumns="False"
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderStyle-CssClass="sorting_mod editacc">
                                        <HeaderTemplate>
                                            Edit</HeaderTemplate>
                                        <ItemTemplate>
                                            <asp:LinkButton ID="lnkEdit" runat="server" Text="Edit" CommandArgument='<%# Eval("HostelID") %>'
                                    ...
                                    <asp:TemplateField HeaderStyle-CssClass="sorting_mod deleteacc">
                                        ...
                                        <ItemTemplate>
                                            <asp:LinkButton ID="lnkDelete" runat="server" Text="Delete" CommandArgument='<%# Eval("HostelID") %>'
